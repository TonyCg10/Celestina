...
**The lock.** Used from `Mod+Shift+A` repeatedly in daily use: every output
covered, a wrong passphrase leaving the session locked, and the correct one
returning it. Recorded as `VAL-R6` passed. What the author did not separately
exercise — killing the shell or the lock process while locked, an output
arriving mid-lock, and `lock-and-suspend`'s refusal path — stays on the
automated regressions' account, not an author-watched one.
