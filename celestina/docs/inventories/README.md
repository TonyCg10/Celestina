# Celestina delivery inventories

This is the stable home for exact commit inventories owned by Celestina. Store
each record as `<plan-slug>/<unit>.numstat.tsv`, where `plan-slug` is the plan's
exact basename without `.md` and `unit` is the exact ledger id.
