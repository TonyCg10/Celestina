...
`close` runs `git fetch origin main` and refuses while
`unit/<project>/<unit>` has a commit that is not reachable from `origin/main`,
or while the worktree holds any change other than the two files `open` wrote.
Otherwise it removes those two files and the worktree, then deletes the
branch. `close` compares commits, not content: the landing publishes a new
sealed commit, so after a landing the session's own commits are still not on
`origin/main` and `close` refuses. Once `land-unit.py` has printed `landed`,
remove that worktree with `git worktree remove <path>` and the branch with
`git branch -D unit/<project>/<unit>`.
...
`check` keeps working there because the guards call it. `land-unit.py` refuses
to run in a session worktree as well.
...
1. **`preflight`.** Stops when the landing worktree already exists, when the
   ...
   rules of the previous section, when a changed path lies outside the row
   prefix's registered commit scope, when the branch changes an inventory that
   exists on `origin/main`, or when a `suite` unit asks for `bug`,
...
6. **`build_if_stale`.** For a project unit, the owning project; for a `suite`
   unit, every registered project whose production inputs hold a path the unit
   changed. For each, it runs
   ...
   built. Otherwise it runs the project's registered `complete_script` when
   the project is deployable, or its `verify_script` when it is not; a
   non-zero exit stops the landing. A `verify_script` verifies an existing
   current build and does not build, so a library whose production inputs
   changed stops here until its `build-production.sh` has run in the canonical
   checkout; `--continue` then verifies. A `suite` unit that changes no
   registered production input runs nothing.
7. **`seal`.** On the canonical checkout, the index holds the rebased tip and
   ...
   - **Build:** <per built project: <entry> exit 0, manifest git_revision <revision>>
   ```
...
8. **`run_guards`.** With the unit staged, the hooks' own commands in order:
   ...
   with its output.
9. **`commit_and_push`.** One `git commit -m <subject>` through the
...
`preflight`, `rebase`, `build_if_stale`, `seal`, `guard`, `commit` or `push`.
