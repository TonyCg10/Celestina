...
| AUD-1-C | `suite:` | active | `scripts/landing.py` (`discover_unit`, `settled_rows`, `plan_settled`, `merge_plan`, `merge_settled_plan`, `merge_other_evidence`, `other_unit_record`, `SEAL_COLUMNS`); `scripts/land-unit.py` (`resolve_unit`, `stacked_on`, `carried_paths`, `hot_merge`); `scripts/test-land-unit.py`; `scripts/worktree.sh` (`open --from`); `scripts/test-worktree.sh`; `docs/contracts/landing.md`; `docs/superpowers/specs/2026-09-25-parallel-unit-landing-design.md` (§10); `docs/evidence/2026-09-26-monorepo-hardening-stacked-landing.md` | — | Support stacked unit branches in the landing (P-0b, ruling R-A1) | [evidence](../../evidence/2026-09-26-monorepo-hardening-stacked-landing.md): `bash scripts/test-land-unit.sh` (same-plan, cross-prefix and two-dependency stacked fixtures, dependency landed and not), `sh scripts/test-worktree.sh`, `sh scripts/check-documentation-contract.sh` | None |
