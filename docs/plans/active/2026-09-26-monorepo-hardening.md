...
| AUD-1-B | `suite:` | active | `celestina-style/scripts/check-style-contract.sh`; `scripts/test-version-contract.py`; `scripts/test-architecture-scanners.sh`; `.github/workflows/contracts.yml`; `.github/workflows/README.md`; `hematita/qml/components/HistoryGraph.qml`; `hematita/qml/components/PathCrumbs.qml` | — | Derive the style-guard roots and the version-owner fixture from `docs/projects.toml`. Fix the two Hematita token violations in the same commit, because a guard coverage change and the violations it reveals must land together or the published revision is red. Add the three missing tests to `contracts.yml`. If the Hematita token fix changes what a person sees, the implementer reports it and the unit lands as `suite-bug` bumping Hematita (ruling R-A6). (P-1: TOOL-1, TOOL-2, TOOL-13) | Every `contracts.yml` step run locally in order, exit 0, with RED and GREEN for both CI failures and the widened style guard: [evidence](../../evidence/2026-09-26-monorepo-hardening-ci.md); GitHub `contracts` green on the landed commit | None |
| AUD-1-C | `suite:` | planned | `scripts/land-unit.py`; `scripts/landing.py` (`discover_unit`, `merge_plan`); `scripts/test-land-unit.py`; `scripts/test-land-unit.sh`; `docs/contracts/landing.md` | — | Let `scripts/land-unit.py` accept a stacked branch: a ledger row the branch carries that equals `origin/main`'s version of that row is accepted instead of stopping the plan merge, and another unit's evidence file in an add/add conflict takes `main`'s copy. Add fixture tests for both cases. (P-0b: no audit finding; ruling R-A1) | `bash scripts/test-land-unit.sh`, `python3 scripts/test-land-unit.py` with a stacked-branch fixture whose dependency already landed; `sh scripts/check-documentation-contract.sh` | None |
