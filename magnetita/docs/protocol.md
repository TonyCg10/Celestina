...
| 10 | sms | 1 conversations `{0 [{0 thread u64, 1 addresses [text], 2 snippet, 3 timestamp_ms, 4 unread u16}]}`; 2 thread request `{0 thread, 1 before_ms?, 2 limit u16 in 1..=256}`; 3 thread `{0 thread, 1 [message]}`; 4 send `{0 thread, 1 body non-empty}`; 5 received `{0 thread, 1 message}` |
