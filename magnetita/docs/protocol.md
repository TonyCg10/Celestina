...
| 9 | mirror | 1 start `{0 max_size u16, 1 fps u8 > 0, 2 bitrate_kbps u32, 3 codec u8, 4 audio}`; 2 started `{0 width, 1 height, 2 codec, 3 audio?}`; 3 stop `{}`; 4 touch `{0 action u8, 1 x, 2 y, 3 pointer u8}`; 5 key `{0 android keycode u16, 1 pressed}`; 6 global `{0 action u8: back, home, recents}`; 7 keyframe `{}` (desktop asks for a key frame now) |
