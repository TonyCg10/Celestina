...
| MAG-S1-C | `magnetita:` | done | [inventory](../../inventories/2026-08-05-network-input-hardening/MAG-S1-C.numstat.tsv) | 8 files, +91/-3 | Pace the one arm of the TLS handshake loop that retried without waiting, so a non-blocking `Ok` while still handshaking cannot spin a link pump thread at full CPU until the deadline | `cargo test -p magnetita-net`, `cargo fmt --all --check`, `cargo clippy -p magnetita-net --all-targets --locked -- -D warnings` — recorded in [handshake pacing evidence](../../evidence/2026-08-06-handshake-pacing.md) | `VAL-MAG-HARDENING` |
