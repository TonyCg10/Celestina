# Celestina Rust workspace delivery inventories

This is the stable home for exact commit inventories owned by the Rust
workspace. Store each record as `<plan-slug>/<unit>.numstat.tsv`, where
`plan-slug` is the plan's exact basename without `.md` and `unit` is the exact
ledger id.
...
units and links. A separate archive commit uses a new administrative unit and
