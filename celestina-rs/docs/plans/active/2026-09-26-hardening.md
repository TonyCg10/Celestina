...
|---|---|---|---|---|---|---|---|
| RS-H1-A | `celestina-rs:` | active | `celestina-rs/crates/celestina-core/src/` (`atomic_file.rs`, `xdg.rs`, new `file_uri.rs`, `desktop_entry.rs`, `lib.rs` crate doc, cancellation and test scratch, `image.rs` doc); `celestina-rs/crates/celestina-shell-core/src/lib.rs` (crate doc); `celestina-rs/README.md`; `celestina-rs/AGENTS.md`; `celestina-rs/STATUS.md`; `celestina-rs/docs/evidence/2026-09-26-shared-owners.md` | — | Add dormant owners: a strict `file_uri` → `PathBuf` parser, `xdg::runtime_dir()` with no `/tmp` fallback, private-state and media-landing writers, `read_bounded`, a truthful post-rename outcome, `desktop_entry::{read, scan, find}` and `application_search_dirs` (regular file, 64 KiB, one shadowing rule), and a non-blocking cancellation query. Correct the workspace README, AGENTS and crate docs. The unit is purely additive (ruling R-A3): no existing function changes behaviour, so RS-10's value unescaping and any change to `is_cancelled` land in a consumer's bug unit. (P-6: Owner halves of RS-1, RS-2/FLU-2, RS-3, RS-4, RS-6, RS-9, RS-10; RS-7; RS-19) | [Shared owners](../../evidence/2026-09-26-shared-owners.md): `cargo test -p celestina-core --offline --locked` (82 passed, 49 new: localhost vs foreign host, `%XX`, NUL, non-UTF-8 bytes; relative runtime dir refused; file 0600, dirs 0700, mode preserved, no-replace refuses; FIFO and oversize refused; fsync-failure outcome; shadowing and cap; hermetic dir order; pause-after-cancel); `cargo clippy --workspace --all-targets -- -D warnings`. No consumer switches, so there are no product bumps. The landing rebuilds every linking app. | None |
