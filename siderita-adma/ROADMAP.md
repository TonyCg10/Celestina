...
- [x] Watcher wired to `WatchState` (invalidate + rescan wins) — a `notify`-backed (inotify) debouncer (full, event-kind-aware) watches the current folder non-recursively and coalesces bursts (200 ms); a change marshals to the Qt thread, `WatchState::observe_change` marks the snapshot stale, and a fresh rescan wins (read-only, so no feedback loop). Navigation moves the watch; a rescan of the same folder just `mark_rescanned`s it. A lost watch (`degrade`) flips a truthful "⚠ Vigilancia perdida · instantánea" status. Verified end-to-end against real create/remove events
