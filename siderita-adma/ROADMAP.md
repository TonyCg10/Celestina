...
- [ ] Replace `QStringList` with a native role-based `QAbstractListModel`, dropping the per-delegate token/kind/subtitle invokables and the `viewRevision` workaround — **deferred as its own pass** (the current model works correctly; this is a cleanliness/perf item). Scoped 2026-07: cxx-qt 0.9.1 offers no `QAbstractListModel` virtual overrides (`rowCount`/`data`/`roleNames`) or `beginResetModel` from Rust — only subclassing a *concrete* model (its example uses `QStringListModel`) — so a native role model means a hand-written C++ `QAbstractListModel` subclass plus cross-QObject wiring plus a ~50-site QML migration. Worth doing deliberately, not folded into other work
