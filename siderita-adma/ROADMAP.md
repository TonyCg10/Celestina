...
- [~] Full accessibility (screen reader, focus order, contrast, animations) and daily-use resource budgets — the new CP2 surfaces are labelled: the Papelera sidebar entry is an accessible button, the conflict / Trash / Abrir-con overlays are `Accessible.Dialog`s, and their list rows carry names + selected state (the existing list/grid rows and controls already did). A full screen-reader pass, focus-order audit, contrast/reduced-motion review and the daily-use budgets remain — none verifiable without a real session
