...
- [~] Drag-and-drop to move / copy within the view and to and from other applications — **drop-in done**: files dragged from another application (or browser) drop into the current folder, or onto a folder row/cell to land inside it (list + grid), default copy with Shift to move — routed through the same collision-detection + async worker + conflict dialog as paste (`controller.drop_uris`). Still open: dragging entries **out** to other apps and **internal** move-by-drag between folders, which need the existing bookmark-only row drag reworked to carry a `text/uri-list`. (Drop interaction builds/qmllint/loads clean; live drag awaits a Wayland session.)
