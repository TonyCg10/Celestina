...
|---|---|---|---|---|---|---|---|
| STYLE-G7-N | `celestina-style:` | active | `CelestinaScrollBar.qml`; `GlassMenuItem.qml`; `CelestinaSwitch.qml` and `CelestinaTextField.qml` (test `objectName`s); `CelestinaTheme.qml` (swatch and `lockScrim` tokens); `DESIGN.md`; `STATUS.md`; `scripts/check-style-contract.sh` (literal alpha rule); `scripts/check-contrast-contract.py` (lock pairs); `tests/` (`tst_switch.qml`, `tst_textfield.qml`, `tst_scrollbar.qml`, `tst_menuitem.qml`, `tst_icongradient.qml`) | — | Spanish `qsTr` scrollbar names. Swatch tokens, and a guard for literal `withAlpha`/`multiplyAlpha`. A `lockScrim` token plus lock pairs in `check-contrast-contract.py`. `tst_switch.qml`, `tst_textfield.qml`. (P-16: STY-1, STY-2, STY-3; SH-6 (token and contract half)) | [evidence](../../evidence/2026-09-26-tokens-and-lock-contrast.md) | None |
...
| STYLE-G7-G | `celestina-style:` | done | [inventory](../../inventories/2026-08-04-shared-reading-controls/STYLE-G7-G.numstat.tsv); `CelestinaTreemap.qml`; `CelestinaUsageList.qml`; their tests; `qmldir`; `CMakeLists.txt`; `DESIGN.md`; `STATUS.md`; root version history | 12 files, +761/-9 | Publish a shared treemap and usage list extracted from Hematita's local controls, with navigation-only contracts (host-supplied tone colours, marked and dimmed ids; Space and Delete left to the host), so Siderita can become their second consumer; move the module to 1.9.0 | [evidence](../../evidence/2026-09-25-usage-controls.md) | `VAL-STYLE-04` |
...
The literal-alpha rule's negative fixture is recorded in the unit's evidence
rather than in the root `scripts/test-architecture-scanners.sh`, which lies
outside the `celestina-style/` commit root this prefix may change.
