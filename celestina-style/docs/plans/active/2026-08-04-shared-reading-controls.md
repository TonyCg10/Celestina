...
|---|---|---|---|---|---|---|---|
| STYLE-G7-N | `celestina-style:` | active | `CelestinaScrollBar.qml`; `GlassMenuItem.qml`; `CelestinaTheme.qml` (swatch and `lockScrim` tokens); `DESIGN.md`; `STATUS.md`; `scripts/check-style-contract.sh` (literal alpha rule); `scripts/check-contrast-contract.py` (lock pairs); `tests/` (`tst_switch.qml`, `tst_textfield.qml`, `tst_scrollbar.qml`, `tst_menuitem.qml`, `tst_icongradient.qml`) | — | Spanish `qsTr` scrollbar names. Swatch tokens, and a guard for literal `withAlpha`/`multiplyAlpha`. A `lockScrim` token plus lock pairs in `check-contrast-contract.py`. `tst_switch.qml`, `tst_textfield.qml`. (P-16: STY-1, STY-2, STY-3; SH-6 (token and contract half)) | [evidence](../../evidence/2026-09-26-tokens-and-lock-contrast.md) | None |
