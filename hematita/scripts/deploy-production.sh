...
app_id=org.celestina.Hematita
production_require_verified "$suite_root" hematita
production_install_xdg_application \
    "$project_root/target/release/hematita" hematita "$app_id" \
    "$project_root/$app_id.desktop" \
    "$suite_root/celestina-style/icons/apps/$app_id.svg" "$prefix"
echo ">> verified Hematita deployed to $prefix without rebuilding"
