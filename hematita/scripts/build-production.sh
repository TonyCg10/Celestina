...
if [ "$#" -eq 0 ]; then
    exec python3 "$artifact_tool" run-build hematita
fi
...
(cd "$project_root" && cargo build --release --locked --bin hematita)

echo ">> Hematita release build steps completed (not installed)"
