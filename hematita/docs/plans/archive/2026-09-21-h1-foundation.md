...
| H1-D | `hematita:` | done | [inventory](../../inventories/2026-09-21-h1-foundation/H1-D.numstat.tsv) | 9 files, +147/-28 | The histories ship as `QVariant` properties carrying a variant list, because `qmllint` cannot resolve `QList_f64` (four `unresolved-type` warnings, quoted in the H1-D evidence); no lint suppressions remain; drop the decorative checked state on `ResourceRow`; guard memory before the first sample (the unit's commit subject names the attempted `QList<f64>` revert and is immutable history) | [history type](../../evidence/2026-09-21-h1-history-type.md) | `VAL-H1` |
