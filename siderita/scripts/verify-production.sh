...
python3 "$artifact_tool" check siderita
...
python3 "$artifact_tool" record-verification siderita \
    --verify-command 'scripts/test-production-artifacts.sh' \
    --verify-command 'bash scripts/check-architecture-contract.sh' \
    --verify-command 'cargo fmt/clippy/test --manifest-path siderita/Cargo.toml' \
    --verify-command 'cargo fmt --manifest-path celestina-rs/Cargo.toml --all --check' \
    --verify-command 'cargo clippy --manifest-path celestina-rs/Cargo.toml -p celestina-core -p siderita-core -p siderita-ops -p siderita-qt -p grafita-core -p fluorita-core -p fluorita-engine -p fluorita-qt --all-targets --locked -- -D warnings' \
    --verify-command 'cargo test --manifest-path celestina-rs/Cargo.toml -p celestina-core -p siderita-core -p siderita-ops -p siderita-qt -p grafita-core -p fluorita-core -p fluorita-engine -p fluorita-qt' \
    --verify-command 'scripts/qmllint-cxxqt.sh siderita' \
    --verify-command 'siderita/scripts/qml-tests.sh' \
    --verify-command 'siderita/scripts/smoke.sh --binary siderita/target/release/siderita'

echo ">> Siderita vigente y verificada; lista para deploy sin recompilar"
