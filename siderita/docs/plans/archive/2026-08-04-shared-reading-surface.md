...
SID-G7-I is the qmllint ratchet doing its job on the unit before it. Asking the
adapter from inside the tab delegate means reaching an outer id, and resolving
the label on the view and reading it back through `ListView.view` costs more
warnings still; either way the file would have pushed past a ceiling that exists
so this debt only falls. The heading keeps its adapter call because it cost
nothing. What the strip needs is to be handed its label rather than to reach for
one, which is a change to how it is fed.
