...
# run.sh — build Fluorita in release and install it into the user's XDG prefix
# (~/.local): a binary on PATH, the desktop entry the launcher and "Abrir con"
# list, and the icon in the hicolor theme.
#
# The binary on PATH is what makes Siderita's `Enter` on a media file open this
# player: that activation launches `fluorita` by name and falls back to the
# desktop's own handler when it is not there.
#
# Installing does *not* make Fluorita the default for any type. Appearing in
# "Abrir con" is the desktop's invitation; taking the default without being
# asked is not.
#
...
Compila Fluorita en release y la instala en el prefijo XDG (por defecto ~/.local):
  bin/fluorita, share/applications/, share/icons/hicolor/…
...
if [ "$uninstall" -eq 1 ]; then
    rm -f "$bin_dir/fluorita" "$apps_dir/$app_id.desktop"
...
binary=$repo_root/target/release/fluorita
if [ ! -x "$binary" ]; then
...
# ── Binary ───────────────────────────────────────────────────────────────────
mkdir -p "$bin_dir"
install -m 0755 "$binary" "$bin_dir/fluorita"
...
echo ">> Fluorita compilada e instalada en $prefix" >&2
echo "   binario: $bin_dir/fluorita" >&2
case ":$PATH:" in
    *":$bin_dir:"*) ;;
    *) echo "   aviso: $bin_dir no está en PATH" >&2 ;;
esac
echo "   ábrela desde el launcher o con: fluorita RUTA" >&2
